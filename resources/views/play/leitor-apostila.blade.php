...
<html>
    ...
    <script>
        ...
        function habilitarMarcacaoPin()
        {
            ...
            else
            {
                var popupMarcacao = `<div id="divPopUpMarcacaoHabilitada" style="position: fixed;right: 0px;z-index: 9999;background: rgba(0, 0, 0, 0.85);color: white;padding: 20px;font-size: 16px;border-radius: 10px;margin: 10px;transition: all 0.3s ease-in 0s;top: 0px;opacity: 1;max-width: 200px;text-align: center;">
                    ...
                    Marcação habilitada, clique no conteúdo do livro digital ou revista para realizar uma anotação!
                </div>`;
        ...
        {
            // console.log($(e.target).hasClass('textLayer'))
            // console.log("Clickou");

            console.log(parent.$('meta[name="csrf-token"]').attr('content'));

            if(!marcandoPin)
            {
                // console.log("Não está marcando pin");

                return;
            }

            // if(e.target.id != ("page" + getPaginaAtual()) && $(e.target).hasClass('textLayer') == false && $(e.target.parentNode).hasClass('textLayer') == false)
            // {
            //     console.log("Click em local não permitido");
            //     //console.log(e.target);
            //     //console.log($(e.target).closest('.textLayer')[0]);
            //     //return;
            // }
            ...
            //$(indicador).find('.btn-deletar-anotacao').on("click", () => console.log("teste"));
        ...
        function popularAnotacoesPaginaAtual()
        {
            ...
            // console.log(anotacoesPaginaAtual);
                    ...
                    $.ajax({
                        ...
                        success: function( _response )
                        {
                            ...
                            if(_response.success != null)
                            {
                                ...
                                console.log(parent.$("#divAnotacao" + anotacao_id), anotacao_id);
                                console.log(parent.$("#divAnotacao" + anotacao_id).length);
        ...
        function marcarPagina()
        {
            ...
            {
                console.log("Não é possível marcar a página, conteudo_id inválido!");
                return;
            }
